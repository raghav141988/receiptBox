...
import {fetchUnknownReceipts,resetReceiptDetail,moveToMyReceipts,deleteReceipts} from '../store/actions/receipts';
...
const MOVE_RECEIPTS_TEXT='Move all emails to my receipts';
  ...
  class UnknownReceipts extends Component {
    NAVBAR_HEIGHT=100;
...
const mapStateToProps = state => {
    return {
      receipts: state.receipts.unknownReceipts,
  ...
  const mapDispatchToProps = dispatch => {
    return {
      ...
      onMoveLatestReceipts:(receipts)=>dispatch(moveToMyReceipts(receipts,false)),
      onMyReceipts: () => dispatch(fetchUnknownReceipts()),
